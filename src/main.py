...
def main():
    ...
    FPS_CLOCK = pygame.time.Clock() #Start the frames clock
    DISPLAY = pygame.display.set_mode((WINDOWWIDTH, WINDOWHEIGHT)) #Set the window size
    pygame.display.set_caption('Battleship') #Set window name
    ...
    game_setup() #Runs the part of the game where the user places their ships
    run_game()
...
# Returns two arrays containing 10 arrays (one for each row) of Tile object, for the user grid and computer grid
def generate_grids():
    user_grid = []
    computer_grid = []
